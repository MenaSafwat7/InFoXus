...
import mina.infoxus.databinding.ActivityAddTimedActionActivityBinding
import mina.infoxus.databinding.CheatHourItemBinding
import mina.infoxus.databinding.DialogAddTimedActionBinding
...
class TimedActionActivity : AppCompatActivity() {
    ...
    private fun makeCheatHoursDialog(editingItem: AutoTimedActionItem? = null, editingPosition: Int = -1) {

        dialogAddToTimedActionBinding = DialogAddTimedActionBinding.inflate(layoutInflater)

        val startTime = if (editingItem != null) {
            TimeRangePicker.Time(editingItem.startTimeInMins / 60, editingItem.startTimeInMins % 60)
        } else {
            TimeRangePicker.Time(6, 30)
        }
        
        val endTime = if (editingItem != null) {
            TimeRangePicker.Time(editingItem.endTimeInMins / 60, editingItem.endTimeInMins % 60)
        } else {
            TimeRangePicker.Time(22, 0)
        }
        ...
        if (editingItem != null) {
            dialogAddToTimedActionBinding.cheatHourTitle.setText(editingItem.title)
            selectedUnblockedApps = ArrayList(editingItem.packages)
            dialogAddToTimedActionBinding.btnSelectUnblockedApps.text =
                getString(R.string.app_s_selected, selectedUnblockedApps?.size ?: 0)
        } else {
            selectedUnblockedApps = arrayListOf()
        }
        ...
        when (selectedMode) {
            ...
            MODE_APP_BLOCKER_CHEAT_HOURS -> {
                ...
                dialogAddToTimedActionBinding.btnSelectUnblockedApps.text = "Select Apps to Unblock"
            }
        }
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(if (editingItem != null) "Update" else getString(R.string.add)) { dialog, _ ->
                if (dialogAddToTimedActionBinding.cheatHourTitle.text?.isEmpty() == true) {
                    ...
                    if (editingItem != null && editingPosition != -1) {
                        timedActionList[editingPosition] = newItem
                        binding.recyclerView2.adapter?.notifyItemChanged(editingPosition)
                    } else {
    ...
    inner class CheatHourAdapter(
        ...
        inner class CheatHourViewHolder(private val binding: CheatHourItemBinding) :
            ...
            fun bind(item: AutoTimedActionItem, position: Int) {
                ...
                binding.removeCheatHour.setOnClickListener {
                    ...
                    if (selectedMode == MODE_AUTO_FOCUS && isAntiUninstallOn) {
                        Toast.makeText(this@TimedActionActivity, "Auto-Focus modes cannot be deleted while Anti-Uninstall is active", Toast.LENGTH_SHORT).show()
                        return@setOnClickListener
                    }
                    if (position < 0 || position >= timedActionList.size) return@setOnClickListener
                    timedActionList.removeAt(position)
    ...
    private fun saveList() {
        when (selectedMode) {
            ...
            MODE_AUTO_FOCUS -> {
                ...
                sendBroadcast(Intent(MainAccessibilityService.ACTION_REFRESH_FOCUS_MODE))
            }
        }
    }
