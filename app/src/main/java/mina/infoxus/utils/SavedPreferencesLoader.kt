...
import mina.infoxus.data.AttentionSpanVideoItem
...
class SavedPreferencesLoader(private val context: Context) {
    ...
    fun loadBlockedSites(): Set<String> {
        val sharedPreferences =
            context.getSharedPreferences("app_preferences", Context.MODE_PRIVATE)
        return sharedPreferences.getStringSet("blocked_sites", emptySet()) ?: emptySet()
    }
    ...
    fun saveBlockedSites(sites: Set<String>) {
        val sharedPreferences =
            context.getSharedPreferences("app_preferences", Context.MODE_PRIVATE)
        sharedPreferences.edit().putStringSet("blocked_sites", sites).apply()
    }

    fun loadIsAdultSitesBlockerEnabled(): Boolean {
        val sharedPreferences =
            context.getSharedPreferences("app_preferences", Context.MODE_PRIVATE)
        return sharedPreferences.getBoolean("is_adult_sites_blocker_enabled", false)
    }

    fun saveIsAdultSitesBlockerEnabled(isEnabled: Boolean) {
        val sharedPreferences =
            context.getSharedPreferences("app_preferences", Context.MODE_PRIVATE)
        sharedPreferences.edit().putBoolean("is_adult_sites_blocker_enabled", isEnabled).apply()
    }
    ...
    fun saveAutoFocusHoursList(cheatHoursList: MutableList<TimedActionActivity.AutoTimedActionItem>) {
        ...
        DataBackupManager.backupAutoFocusHours(context)
    }
    ...
    fun saveFocusModeData(focusModeData: FocusModeBlocker.FocusModeData) {
        ...
        // Backup the data to file
        DataBackupManager.backupFocusMode(context)
    }
    ...
    fun saveUsageLimits(config: UsageLimitsConfig) {
        val sharedPreferences = context.getSharedPreferences("usage_limits", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        val gson = Gson()
        val json = gson.toJson(config)
        editor.putString("usage_limits_config", json)
        editor.apply()
        
        // Backup the data to file
        DataBackupManager.backupUsageLimits(context)
    }

    fun loadUsageLimits(): UsageLimitsConfig {
        val sharedPreferences = context.getSharedPreferences("usage_limits", Context.MODE_PRIVATE)
        val gson = Gson()
        val json = sharedPreferences.getString("usage_limits_config", null)
        if (json.isNullOrEmpty()) return UsageLimitsConfig()
        val type = object : TypeToken<UsageLimitsConfig>() {}.type
        return gson.fromJson(json, type)
    }
}
