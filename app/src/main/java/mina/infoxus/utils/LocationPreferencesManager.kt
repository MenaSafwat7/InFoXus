...
import mina.infoxus.data.LocationBlockerConfig
...
class LocationPreferencesManager(private val context: Context) {
    ...
    private fun saveLocations(locations: List<SavedLocation>) {
        ...
        // Backup the data to file whenever locations are saved
        DataBackupManager.backupLocations(context)
    }
    ...
    fun saveConfig(config: LocationBlockerConfig) {
        prefs.edit().apply {
            ...
            putBoolean(KEY_AUTO_CONTROL_FOCUS_MODE, config.autoControlFocusMode) // Save auto-control focus mode
            commit()
        }
    ...
    fun setEnabled(enabled: Boolean) {
        ...
        // Backup the data to file when enabled state changes
        DataBackupManager.backupLocations(context)
    }
    ...
    fun setGlobalMode(mode: LocationMode) {
        prefs.edit().putString(KEY_GLOBAL_MODE, mode.name).commit()
        
        // Backup the data to file when mode changes
        DataBackupManager.backupLocations(context)
    }
    ...
    fun setAutoControlFocusMode(enabled: Boolean) {
        prefs.edit().putBoolean(KEY_AUTO_CONTROL_FOCUS_MODE, enabled).commit()
        
        // Backup the data to file when auto-control focus mode changes
        DataBackupManager.backupLocations(context)
    }
