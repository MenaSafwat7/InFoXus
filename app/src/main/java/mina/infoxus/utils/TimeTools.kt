...
class TimeTools {
    companion object {
        ...
        fun getUsageCycleLocalDate(resetHour: Int = 5, resetMinute: Int = 0): LocalDate {
            val now = java.time.LocalDateTime.now()
            // Reset is at configurable time (default 5:00 AM)
            val resetTime = now.toLocalDate().atTime(resetHour, resetMinute)
        ...
        fun getUsageCycleDate(resetHour: Int = 5, resetMinute: Int = 0): String {
            val cycleDate = getUsageCycleLocalDate(resetHour, resetMinute)
            val formatter = DateTimeFormatter.ofPattern("dd MMMM yyyy")
            return cycleDate.format(formatter)
        }
    }
}
